...
class BatchDetailViewTest(TestCase):
    ...
    def test_initial_empty_batch_authenticated_request(self):
        ...
        self.assertEqual(batch['status'], {'code': 0, 'display': 'Initial'})
        self.assertEqual(batch['summary'], {'initial_commands': 0, 'running_commands': 0, 'done_commands': 0, 'total_commands': 0})
    ...
    def test_batch_authenticated_request(self):
        ...
        self.assertEqual(batch['status'], {'code': 1, 'display': 'Running'})
        self.assertEqual(batch['summary'], {'initial_commands': 1, 'running_commands': 0, 'done_commands': 0, 'total_commands': 1})
        self.assertEqual(batch['message'], None)
        self.assertTrue('created' in batch)
        self.assertTrue('modified' in batch)
        self.assertEqual(batch['commands_url'], f'http://testserver/api/v1/batches/{original.pk}/commands/')
        ...
        self.assertEqual(batch['name'], 'Batch 1')
        self.assertEqual(batch['status'], {'code': 1, 'display': 'Running'})
        self.assertEqual(batch['summary'], {'initial_commands': 0, 'running_commands': 1, 'done_commands': 0, 'total_commands': 1})
        self.assertEqual(batch['message'], None)
        self.assertTrue('created' in batch)
        self.assertTrue('modified' in batch)
        self.assertEqual(batch['commands_url'], f'http://testserver/api/v1/batches/{original.pk}/commands/')
        ...
        self.assertEqual(batch['name'], 'Batch 1')
        self.assertEqual(batch['status'], {'code': 1, 'display': 'Running'})
        self.assertEqual(batch['summary'], {'initial_commands': 0, 'running_commands': 0, 'done_commands': 1, 'total_commands': 1})
        self.assertEqual(batch['message'], None)
        self.assertTrue('created' in batch)
        self.assertTrue('modified' in batch)
        self.assertEqual(batch['commands_url'], f'http://testserver/api/v1/batches/{original.pk}/commands/')
        ...
        self.assertEqual(batch['name'], 'Batch 1')
        self.assertEqual(batch['status'], {'code': 1, 'display': 'Running'})
        self.assertEqual(batch['summary'], {'initial_commands': 1, 'running_commands': 0, 'done_commands': 1, 'total_commands': 2})
        self.assertEqual(batch['message'], None)
        self.assertTrue('created' in batch)
        self.assertTrue('modified' in batch)
        self.assertEqual(batch['commands_url'], f'http://testserver/api/v1/batches/{original.pk}/commands/')
        ...
        self.assertEqual(batch['name'], 'Batch 1')
        self.assertEqual(batch['status'], {'code': 1, 'display': 'Running'})
        self.assertEqual(batch['summary'], {'initial_commands': 1, 'running_commands': 1, 'done_commands': 1, 'total_commands': 3})
        self.assertEqual(batch['message'], None)
        self.assertTrue('created' in batch)
        self.assertTrue('modified' in batch)
        self.assertEqual(batch['commands_url'], f'http://testserver/api/v1/batches/{original.pk}/commands/')
